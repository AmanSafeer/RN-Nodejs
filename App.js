...
const App = () => {
  ...
  const sendMessage = () => {
    if (name && message) {
      Axios.post(
        'http://localhost:3000/send_messages',
        { name, message },
  ...
  const getMessages = () => {
    Axios.get('http://localhost:3000/get_messages')
      .then(res => {
  ...
  const updateMessage = () => {
    ...
    Axios.put(
      "http://localhost:3000/update_messages",
      payload,
      { headers: { 'Content-Type': 'application/json' } }
    )
      .then(res => {
  ...
  const deleteMessage = (_id) => {
    Axios.delete(
      "http://localhost:3000/delete_messages",
  ...
  return (
    <SafeAreaView style={{ flex: 1, }} >
      <View style={{ flex: 1, padding: 20 }}>
        ...
        {!edit ?
          <TouchableOpacity onPress={sendMessage} style={{ marginVertical: 15, padding: 10, alignSelf: "center", backgroundColor: "lightgray" }}>
            <Text>Send</Text>
          </TouchableOpacity>
          :
          <TouchableOpacity onPress={updateMessage} style={{ marginVertical: 15, padding: 10, alignSelf: "center", backgroundColor: "lightgray" }}>
            <Text>Update</Text>
          </TouchableOpacity>
        }
        <FlatList
